...
export interface PlatformClient {
    ...
    getDiffs(config: AppConfig, repoId: string, baseCommit: string, headCommit: string, options?: {
        maxFiles?: number;
        maxFileSize?: number;
        prId?: number;
    }): Promise<string>;
    listPrThreads(config: AppConfig, repoId: string, prId: number): Promise<PrThread[]>;
    createPrThread(config: AppConfig, repoId: string, prId: number, content: string, inlineContext?: InlineCommentContext, iterationContext?: {
        firstComparingIteration: number;
        secondComparingIteration: number;
    }): Promise<PrThread>;
    deletePrThreadComment(config: AppConfig, repoId: string, prId: number, threadId: number, commentId: number): Promise<void>;
}
