...
exports.listPrThreads = listPrThreads;
exports.deletePrThreadComment = deletePrThreadComment;
exports.createPrThread = createPrThread;
exports.getPrIterations = getPrIterations;
...
async function fetchPr(config, repoId, prId) {
    ...
    return parseJsonOrThrow(res, config, `fetchPr(${prId})`);
}
async function listPrThreads(config, repoId, prId) {
...
async function deletePrThreadComment(config, repoId, prId, threadId, commentId) {
...
async function createPrThread(config, repoId, prId, content, inlineContext, iterationContext) {
    const url = buildUrl(config.orgUrl, config.project, `/repositories/${repoId}/pullRequests/${prId}/threads`);
    const body = {
...
async function getPrIterations(config, repoId, prId) {
...
async function getPrChanges(config, repoId, prId, iterationId) {
...
async function getDiffs(config, repoId, baseCommit, targetCommit, maxFiles = 50, maxFileSize = 100000) {
